...
ptconception<-amovahyps$ptconception
capemendo<-amovahyps$capemendo
capeblanco<-amovahyps$capeblanco
pop1<-as.factor(pop)
pop1
...
```
```{r}
amova_out<-pegas::amova(formula=dists~capeblanco/pop1,nperm=1)
...
cape_blanco_result<-c(FCT,FSC,FST)
```


```{r}
AMOVA_result<-cbind(c("Among Regions","Among Populations within Regions", "Among Populations"),pt_conception_result,cape_mendo_result,cape_blanco_result)
