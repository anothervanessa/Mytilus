...
myt_aln<-read.FASTA("mcali_seqs_210506.fasta")
myt_aln
...
myt_amova<-read.FASTA("sorted_seqs.fasta")
...
ptconception<-amovahyps$pt_conception
capemendo<-amovahyps$cape_mendo
marko<-amovahyps$marko_us
pop1<-as.factor(pop)
pop1
...
```{r}
amova_out<-pegas::amova(formula=dists~ptconception/pop1,nperm=1)
...
```{r}
amova_out<-pegas::amova(formula=dists~capemendo/pop1,nperm=1)
...
#Test Marko Hypothesis
```{r}
amova_out<-pegas::amova(formula=dists~marko/pop1,nperm=1)
...
marko_result<-c(FCT,FSC,FST)
```



```{r}
AMOVA_result<-cbind(c("Among Regions","Among Populations within Regions", "Among Populations"),pt_conception_result,cape_mendo_result, marko_result)

kable(AMOVA_result)
```
#Isolation by Distance
```{r}
myt_metadata<-read.csv("myt_metadata_2.csv")
...
myt_unique<-pcoel_unique[order(pcoel_unique$locality),] #put them in alphabetical order
