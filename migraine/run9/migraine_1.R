...
GP$FONKgScale <- c("logscale","logscale","Canonical")
names(GP$FONKgScale) <- GP$FONKgNames
GP$samplingSpace<-c("twoNmu","twoNm","g") ## ONLY for next points
GP$samplingScale<-c("logscale","logscale","Canonical") ## ONLY for next points
...
GP$oneDimCIvars<-c("twoNmu","twoNm","Nb") ## (an empty c() value will be meaningful for nextBounts)
GP$spec1DProfiles<-c("twoNmu","twoNm","Nb","g")
GP$spec2DProfiles<-c()
GP$S2factor<-1 ##  other values add more confusion that anything else
GP$D2IBDbool<-FALSE;GP$D1IBDbool<-TRUE 
GP$Nbfactor<-49.3506 ##  correction from 2NmS2 to 2Ds2=Nb
...
write("Data file: ../Mcalifornianus_210524-GP.txt",file=GP$cleanResu)
